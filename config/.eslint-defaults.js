 ...
 * eslint有三种使用方式
 * 【1】js代码中通过注释的方式使用
 * 【2】通过package.json的eslintConfig字段设置，eslint会自动搜索项目的package.json文件中的配置
...
module.exports = {
   ...
   *  es2017 - 添加所有 ES2017 中的全局变量且自动设置 ecmaVersion 解析器选项为 8，该版本支持 async 和 await。
   ...
   */
  globals: {
    // process: false, // 例如定义gTool这个全局变量，且这个变量可以被重写
    // require: false,
  },
  ...
  //     'files': ['src/components/icon-box/index.vue'],
  //     'rules': {
  //       'vue/valid-v-bind': 0
  //     }
  //   }
  ...
  rules: {
    ...
    "no-magic-numbers": [1, { // 不需使用魔术字符
      ignoreArrayIndexes: true,
      ignore: [-1, 0, 1, 2, 24, 60, 100, 1000, 1024],
    ...
    "camelcase": 1, // 要求使用骆驼拼写法，因暂时迁就 后端字段，容许下划线命名
    "comma-dangle": [2, 'only-multiline'], // 要求或禁止使用拖尾逗号
    ...
    "space-before-function-paren": [2, { // 禁止函数圆括号之前有一个空格，仅仅 async 后允许添加空格，与主流保持一致
      "anonymous": "never",
      "named": "never",
      'asyncArrow': 'always'
    }],
